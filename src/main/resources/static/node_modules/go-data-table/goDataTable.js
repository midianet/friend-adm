    ...
    return {
        ...
        controller: function ($scope, $element, $attrs, $location, $window) {
            ...
            var getEditBtnTemplate = function (pid) {
                ...
                return  '<button id="btnGODTEditAux" class="btn btn-primary"'+sClick+'><i class="fa fa-pencil"></i></button>';
            };

            var getDeleteBtnTemplate = function (pid) {
                var sClick =  (pid? 'onclick="document.getElementById(\''+getIdDeleteBtn(pid)+'\').click()"' :'');
                return  '<button id="btnGODTDeleteAux" class="btn btn-danger"'+sClick+'><i class="fa fa-trash-o"></i></button>';
            };
	    ...
	    var columns = [
...

                {
                    ...
                    "orderable": false,
                    "data": null,
                    "defaultContent": getDeleteBtnTemplate()
                }
            ];

            this.registraItem = function (data) {
                columns.splice(columns.length - 2, 0, {"data": data.name , "orderable": ((data.notOrderable != null && data.notOrderable == '') ? false : true) });
            };
                    ...
                    "fnRowCallback": function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                       ...
                       var doClick2 = function () {
                        ...
                        if ($(nRow).find('button')){
                            ...
                            var btnEdit = $(nRow).find('#btnGODTEditAux');
                            var btnDelete = $(nRow).find('#btnGODTDeleteAux');

                            $(nRow).find('button').prop('onclick', null).off('click');

                            btnEdit.on("click", doClick);
                            btnEdit.attr("id", getIdEditBtn(aData.id));

                            btnDelete.on("click", doClick2);
                            btnDelete.attr("id", getIdDeleteBtn(aData.id));
